...
def calcular_conductividad_temperatura(material: str, temp_servicio: float) -> float:
    ...
    rho_t = props["rho_20"] * (1 + props["alpha"] * (temp_servicio - 20.0))
    return 1.0 / rho_t
...
def recomendar_seccion_correctora(
    potencia_kw: float, 
    longitud_m: float, 
    tension_str: str, 
    cos_phi: float, 
    ...
    for s in SECCIONES_COMERCIALES:
        ...
        _, dv_pct = calcular_caida_tension(potencia_kw, longitud_m, s, tension_str, cos_phi, material_conductor, aislamiento)
